...
const NewsDetail: React.FC<NewsDetailProps> = ({ post, onBack, onEdit, onDelete }) => {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      try {
        // Since this is an internal tool, window.location.href doesn't
        // point to a shareable public URL. We will share the title and text only.
          ...
          {post.socials && (post.socials.fb || post.socials.insta || post.socials.x) && (
            <div className="text-center">
              ...
              <div className="flex justify-center items-center space-x-8">
                {post.socials.fb && (
                  <a href={post.socials.fb} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-blue-600 transition-colors" aria-label="Facebook">
                    <FacebookIcon />
                  </a>
                )}
                {post.socials.insta && (
                  <a href={post.socials.insta} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-pink-600 transition-colors" aria-label="Instagram">
                    <InstagramIcon />
                  </a>
                )}
                {post.socials.x && (
                  <a href={post.socials.x} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-gray-800 transition-colors" aria-label="X">
                    <XIcon />
                  </a>
                )}
              </div>
