...
const NewsDetail: React.FC<NewsDetailProps> = ({ post, onBack, onEdit, onDelete, onLinkClick, userRole }) => {
  ...
  const handleShare = async () => {
    const articleUrl = window.location.origin;
    const shareData = {
      title: post.title,
    ...
    if (navigator.share) {
      try {
        // For native share, a shorter text and a URL is often better.
        await navigator.share({
          title: shareData.title,
          text: `Check out this article from BJP Himachal Pradesh News Portal: ${post.title}`,
          url: shareData.url,
        });
      } catch (error) {
      ...
      const clipboardText = `${post.title}\n\n${shareData.text}\n\nVisit the portal: ${shareData.url}`;
