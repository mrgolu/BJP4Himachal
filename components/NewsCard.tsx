...
const NewsCard: React.FC<NewsCardProps> = ({ post, onSelectPost, onEditPost, onDeletePost }) => {
  ...
  return (
    <div
      ...
      className="bg-white rounded-lg shadow-lg overflow-hidden cursor-pointer transform hover:-translate-y-1 transition-all duration-300 ease-in-out group relative"
    >
      ...
      {renderMediaPreview()}
      <div className="p-6">
        <span className="inline-block bg-orange-200 text-orange-800 text-xs font-semibold px-2 py-1 rounded-full uppercase tracking-wide mb-2">
          {post.category}
        </span>
        <h3 className="text-xl font-bold text-gray-800 mb-2 group-hover:text-bjp-orange transition-colors">
          {post.title}
        </h3>
        <p className="text-gray-600 text-sm leading-relaxed mb-4">{snippet}</p>
